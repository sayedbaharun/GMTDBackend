...
exports.isAdmin = exports.requireOnboardingComplete = exports.authenticateOptional = exports.authenticateAndSyncUser = exports.syncUserWithDb = exports.validateAccessToken = void 0;
const express_oauth2_jwt_bearer_1 = require("express-oauth2-jwt-bearer");
const types_1 = require("../types");
const auth_1 = require("../services/auth"); // CORRECTED IMPORT PATH
...
const syncUserWithDb = async (req, res, next) => {
    if (!req.auth?.payload?.sub) {
        ...
        logger_1.logger.warn('Auth0 sub missing from req.auth.payload after token validation.');
        ...
        return next(new types_1.ApiError('User identifier missing from token.', 401)); // Changed to 401 as it implies an auth issue
    }
    const auth0UserId = req.auth.payload.sub;
    const email = req.auth.payload.email; // Auth0 email claim
    const name = req.auth.payload.name; // Auth0 name claim
    const emailVerified = req.auth.payload.email_verified; // Auth0 email_verified claim
    try {
        const serviceResponse = await auth_1.authService.getOrCreateUserFromAuth0({
            sub: auth0UserId,
            email: email,
            name: name,
        ...
        if (!serviceResponse.success || !serviceResponse.data) {
            logger_1.logger.error('Failed to get or create user from DB:', serviceResponse.error);
            return next(new types_1.ApiError(serviceResponse.error || 'Could not process user information.', serviceResponse.statusCode || 500));
        }
        req.user = serviceResponse.data; // Attach Prisma user to req.user
        next();
    }
    catch (error) {
        logger_1.logger.error('Error in syncUserWithDb middleware:', error);
        next(new types_1.ApiError('Server error during user synchronization.', 500));
    }
};
...
exports.authenticateAndSyncUser = [exports.validateAccessToken, exports.syncUserWithDb];
...
const authenticateOptional = async (req, res, next) => {
    (0, exports.validateAccessToken)(req, res, (err) => {
        if (err) {
            ...
            logger_1.logger.warn(`Optional auth: Token validation failed or token not provided (proceeding as anonymous): ${err.message}`);
        ...
        if (req.auth?.payload?.sub) {
            (0, exports.syncUserWithDb)(req, res, next).catch(next); // syncUserWithDb will call next() or next(err)
        }
        else {
            ...
            logger_1.logger.warn('Optional auth: Token validated but no user identifier (sub) found in payload. Proceeding as anonymous.');
            next();
        }
    });
};
...
const requireOnboardingComplete = (req, res, next) => {
    ...
    if (!req.user) {
        ...
        return next(new types_1.ApiError('Authentication required to check onboarding status.', 401));
    }
    const user = req.user; // req.user is now PrismaUser from syncUserWithDb
    if (!user.onboardingComplete) {
        return next(new types_1.ApiError('Onboarding not completed. Please complete your profile.', 403));
    }
    next();
};
...
const isAdmin = (req, res, next) => {
    // This middleware must run *after* authenticateAndSyncUser or an equivalent that populates req.user
    if (!req.user) {
        return next(new types_1.ApiError('Authentication required for admin access.', 401));
    }
    const user = req.user; // req.user is now PrismaUser
    if (!user.isAdmin) {
        return next(new types_1.ApiError('Forbidden: Admin access required.', 403));
    }
    next();
};
