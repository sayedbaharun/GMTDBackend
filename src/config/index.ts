...
// Environment validation with different levels
...
const validateEnvironment = (): ValidationResult => {
  ...
  const productionVars = [
    { key: 'DATABASE_URL', value: process.env.DATABASE_URL, required: config.enableDatabase },
  ...
  if (process.env.NODE_ENV === 'production' || !config.demoMode) {
  ...
  if (config.enableDatabase && process.env.DATABASE_URL) {
...
export const startupValidation = () => {
  ...
  console.log(`📊 Database: ${config.enableDatabase ? 'Enabled' : 'Disabled'}`);
  console.log(`🔧 Features: ${Object.entries(config.featureFlags).filter(([_, enabled]) => enabled).map(([name]) => name).join(', ') || 'None'}`);
};
